...
def alpha_003(code,end_date=None):  
    ...
    part2 = (close(code,end_date)-np.minimum(delay1[condition2],low(code,end_date)[condition2])).iloc[-6:,:] #取最近的6位数据
...
def alpha_004(code,end_date=None):
    ...
    condition2=(pd.rolling_sum(close(code,end_date),2)/2<(pd.rolling_sum(close(code,end_date),8)/8-pd.rolling_std(close(code,end_date),8)))
    condition3=(1<=volume(code,end_date)/pd.rolling_mean(volume(code,end_date),20)) 
    indicator1=pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)#[condition2]
    indicator2=-pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)#[condition3]
...
def alpha_009(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp=(high(code,end_date)+low(code,end_date))*0.5-(high(code,end_date).shift()+low(code,end_date).shift())*0.5*(high(code,end_date)-low(code,end_date))/volume(code,end_date) #计算close_{i-1}
...
def alpha_022(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part1=(close(code,end_date)-pd.rolling_mean(close(code,end_date),6))/pd.rolling_mean(close(code,end_date),6)
    temp=(close(code,end_date)-pd.rolling_mean(close(code,end_date),6))/pd.rolling_mean(close(code,end_date),6)
    part2=temp.shift(3)
    result=part1-part2
...
def alpha_025(code,end_date=None):
    ...
    temp_2 = 1-func_decaylinear(volume(code,end_date)/pd.rolling_mean(volume(code,end_date),20),9).rank(axis=1,pct=True)
...
def alpha_031(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    result=(close(code,end_date)-pd.rolling_mean(close(code,end_date),12))*100/pd.rolling_mean(close(code,end_date),12)
    alpha=result.iloc[-1,:]
    return alpha
...
def alpha_048(code,end_date=None):
    ...
    indicator1=pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[condition1].fillna(0)
    indicator2=pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[condition2].fillna(0)
    indicator3=pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[condition3].fillna(0)

    indicator11=-pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[(~condition1)&(close(code,end_date)!=close(code,end_date).shift())].fillna(0)
    indicator22=-pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[(~condition2)&(close(code,end_date).shift()!=close(code,end_date).shift(2))].fillna(0)
    indicator33=-pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[(~condition3)&(close(code,end_date).shift(2)!=close(code,end_date).shift(3))].fillna(0)
...
def alpha_050(code,end_date=None):
    ...
    data1=np.maximum(abs(high(code,end_date)-high(code,end_date).shift()),abs(low(code,end_date)-low(code,end_date).shift()))
    data1[cond1]=0
    part1=pd.rolling_sum(data1,12)
    data2=np.maximum(abs(high(code,end_date)-high(code,end_date).shift()),abs(low(code,end_date)-low(code,end_date).shift()))
    data2[~cond1]=0
...
def alpha_055(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp1 = 16*(close(code,end_date)-close(code,end_date).shift()+(close(code,end_date)-open(code,end_date))/2+close(code,end_date).shift()-open(code,end_date).shift())
...
def alpha_060(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part1=(close(code,end_date).iloc[-20:,:]-low(code,end_date).iloc[-20:,:])-(high(code,end_date).iloc[-20:,:]-close(code,end_date).iloc[-20:,:])
...
def alpha_064(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = pd.rolling_corr(vwap(code,end_date).rank(axis=0,pct=True),volume(code,end_date).rank(axis=0,pct=True),4)
    part_1 = func_decaylinear(temp_1,4)
    cond1=part_1==0
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True)
    temp_2 = pd.rolling_corr(close(code,end_date).rank(axis=0,pct=True),pd.rolling_mean(volume(code,end_date),60).rank(axis=0,pct=True),4)
...
def alpha_066(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = ((close(code,end_date)-pd.rolling_mean(close(code,end_date),6))/pd.rolling_mean(close(code,end_date),6))*100
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_069(code,end_date=None):
    ...
    indicator1 = (pd.rolling_sum(func_dtm(code,end_date),20)- pd.rolling_sum(func_dbm(code,end_date),20))/pd.rolling_sum(func_dtm(code,end_date),20)
    indicator1[(~condition1)&condition2]=0
    indicator1[(~condition1)&(-condition2)]=(pd.rolling_sum(func_dtm(code,end_date),20)- pd.rolling_sum(func_dbm(code,end_date),20))/pd.rolling_sum(func_dbm(code,end_date),20)
...
def alpha_071(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    data = (close(code,end_date)-pd.rolling_mean(close(code,end_date), 24) )/ pd.rolling_mean(close(code,end_date), 24)
...
def alpha_075(code,benchmark='000300.XSHG',end_date=None):
    ...
    part_1=sum(pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[(condition1)&(condition2)].fillna(0).iloc[-50:,:])

    condition3 = benchmark_index_close(code,benchmark,end_date) < benchmark_index_open(code,benchmark,end_date)
    part_2=sum(pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)[(condition3)].fillna(0).iloc[-50:,:])
    alpha = part_1/part_2
    cond1=alpha==0
    alpha[cond1]=nan
    return alpha
...
def alpha_076(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = pd.rolling_std(abs(close(code,end_date) / close(code,end_date).shift(1)-1)/volume(code,end_date),20)
    part_2 = pd.rolling_mean(abs(close(code,end_date)/close(code,end_date).shift(1)-1)/volume(code,end_date),20)
...
def alpha_077(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = (high(code,end_date)+low(code,end_date))/2+high(code,end_date)-vwap(code,end_date)-high(code,end_date)
    part_1 = func_decaylinear(temp_1,20)
    cond1=part_1==0
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True)
    temp_2 = pd.rolling_corr((high(code,end_date)+low(code,end_date))/2,pd.rolling_mean(volume(code,end_date),40),3)
...
def alpha_078(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = (high(code,end_date) + low(code,end_date) + close(code,end_date)) / 3-pd.rolling_mean((high(code,end_date) + low(code,end_date) + close(code,end_date)) / 3, 12)
    temp_1 = abs(close(code,end_date)-pd.rolling_mean((high(code,end_date) + low(code,end_date) + close(code,end_date)) / 3, 12))
...
def alpha_086(code,end_date=None):
    ...
    indicator1=pd.DataFrame(-np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns) 
    indicator2=pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns) 
...
def alpha_096(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = pd.ewma(100*(close(code,end_date)-pd.rolling_min(low(code,end_date),9))/(pd.rolling_max(high(code,end_date),9)-pd.rolling_min(low(code,end_date),9)), span=5, adjust=False)
...
def alpha_099(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = (-pd.rolling_cov(close(code,end_date).rank(axis=1, pct=True), volume(code,end_date).rank(axis=1, pct=True), window=5).rank(axis=0, pct=True)).iloc[-1,:]
    return alpha
...
def alpha_105(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = -pd.rolling_corr(open(code,end_date).rank(axis=0,pct=True),volume(code,end_date).rank(axis=0,pct=True),10)
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_108(code,end_date=None):
    ...
    part_2 = pd.rolling_corr(vwap(code,end_date),pd.rolling_mean(volume(code,end_date),120),6).rank(axis=0,pct=True)
...
def alpha_111(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    data1=volume(code,end_date)*((close(code,end_date)-low(code,end_date))-(high(code,end_date)-close(code,end_date)))/(high(code,end_date)-low(code,end_date))
...
def alpha_113(code,end_date=None):
    ...
    part_3 = pd.rolling_corr(pd.rolling_sum(close(code,end_date),5),pd.rolling_sum(close(code,end_date),20),2).rank(axis=0,pct=True)
...
def alpha_114(code,end_date=None):
    ...
    part_3=(((high(code,end_date)-low(code,end_date))/(pd.rolling_sum(close(code,end_date),window=5)/5))/(vwap(code,end_date)-close(code,end_date))).iloc[-1,:]
...
def alpha_115(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = pd.rolling_corr((high(code,end_date)*0.9+close(code,end_date)*0.1),pd.rolling_mean(volume(code,end_date),30),10).rank(axis=0,pct=True)
...
def alpha_119(code,end_date=None):
    ...
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True)
    temp_2 = pd.rolling_corr(open(code,end_date).rank(axis=0,pct=True),pd.rolling_mean(volume(code,end_date),15).rank(axis=0,pct=True),21)
...
def alpha_122(code,end_date=None):
    ...
    data = pd.ewma(pd.ewma(pd.ewma(np.log(close(code,end_date)), span=12, adjust=False), span=12, adjust=False), span=12, adjust=False)
...
def alpha_128(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    cond1 = ((high(code,end_date)+low(code,end_date)+close(code,end_date))/3) > ((high(code,end_date)+low(code,end_date)+close(code,end_date))/3).shift()
    cond2 = ((high(code,end_date)+low(code,end_date)+close(code,end_date))/3) < ((high(code,end_date)+low(code,end_date)+close(code,end_date))/3).shift()
...
def alpha_130(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = pd.rolling_corr((high(code,end_date)+low(code,end_date))/2,pd.rolling_mean(volume(code,end_date),40),9)
    part_1 = func_decaylinear(temp_1,10)
    cond1=part_1==0
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True)
    temp_2 = pd.rolling_corr(vwap(code,end_date).rank(axis=0,pct=True),volume(code,end_date).rank(axis=0,pct=True),7)
...
def alpha_134(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = (close(code,end_date)-close(code,end_date).shift(12))/close(code,end_date).shift(12) * volume(code,end_date)
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_137(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = (close(code,end_date)-close(code,end_date).shift()+(close(code,end_date)-open(code,end_date))/2+close(code,end_date).shift()-open(code,end_date).shift())*16
    ...
    cond3 = temp_3 >temp_4
    cond4 = temp_3 >temp_2
    data1 = pd.DataFrame(np.ones(close(code,end_date).shape),index=close(code,end_date).index,columns=close(code,end_date).columns)
...
def alpha_138(code,end_date=None):
    ...
    temp_2 = pd.rolling_corr(func_tsrank(low(code,end_date),8),func_tsrank(pd.rolling_mean(volume(code,end_date),60),17),5)
...
def alpha_140(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = (open(code,end_date).rank(pct=True)+low(code,end_date).rank(pct=True)-high(code,end_date).rank(pct=True)+close(code,end_date).rank(pct=True))
    part_1 = func_decaylinear(temp_1,8)
    cond1=part_1==0
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True).T
    temp_2 = pd.rolling_corr(func_tsrank(close(code,end_date),8),func_tsrank(pd.rolling_mean(volume(code,end_date),60),20),8)
...
def alpha_141(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = pd.rolling_corr(high(code,end_date).rank(pct=True),pd.rolling_mean(volume(code,end_date),15).rank(pct=True),9).rank(pct=True)*-1
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_144(code,end_date=None):
    ...
    sumif = ((abs(close(code,end_date) / close(code,end_date).shift()-1)/amount(code,end_date)) * df1).iloc[-20:,:].sum()
...
def alpha_145(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = pd.rolling_mean(volume(code,end_date),9)-pd.rolling_mean(volume(code,end_date),26)/pd.rolling_mean(volume(code,end_date),12) * 100
    alpha = alpha.iloc[-1,:]
    return alpha
...
def alpha_146(code,end_date=None):
    ...
    part_2 = pd.ewma(temp_1,span=60,adjust=False)/pd.ewma((pd.ewma(temp_1,span=60,adjust=False)**2),span=60,adjust=False)
    alpha = part_1.iloc[-1,:] * part_2.iloc[-1,:]
    return alpha
...
def alpha_148(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = pd.rolling_corr(open(code,end_date),pd.rolling_sum(pd.rolling_mean(volume(code,end_date),60),9),6).rank(pct=True)
...
def alpha_149(code,benchmark='000300.XSHG',end_date=None):
    ...
    temp2=benchmark_index_close(code,benchmark,end_date)/benchmark_index_close(code,benchmark,end_date).shift()-1
...
def alpha_156(code,end_date=None):
    ...
    part_1[cond1]=nan
    part_1=part_1.rank(axis = 1,pct=True)
    temp_1 = (open(code,end_date)*0.15+low(code,end_date)*0.85).diff(2)/(open(code,end_date)*0.15+low(code,end_date)*0.85)
...
def alpha_163(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = -1*func_ret(code,end_date)*pd.rolling_mean(volume(code,end_date),20)*vwap(code,end_date)*(high(code,end_date)-low(code,end_date))
...
def alpha_170(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]        
    part_1 = ((1/close(code,end_date)).rank(axis=0,pct=True))*volume(code,end_date)/pd.rolling_mean(volume(code,end_date),20)
...
def alpha_175(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    temp_1 = np.maximum((high(code,end_date)-low(code,end_date)),abs(close(code,end_date).shift()-high(code,end_date)))
...
def alpha_176(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = (close(code,end_date)-pd.rolling_min(low(code,end_date),12))/(pd.rolling_max(high(code,end_date),12)-pd.rolling_min(low(code,end_date),12))
...
def alpha_178(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = (close(code,end_date)-close(code,end_date).shift())/close(code,end_date).shift() * volume(code,end_date)
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_179(code,end_date=None):
    ...
    temp_2 = pd.rolling_corr(low(code,end_date).rank(pct=True),pd.rolling_mean(volume(code,end_date),60).rank(pct=True),12).rank(pct=True)
    alpha = temp_1 *temp_2
    alpha=alpha.iloc[-1,:]
    return alpha
...
def alpha_180(code,end_date=None):
    ...
    sign[sign.iloc[:,:]>0] = 1
    sign[sign.iloc[:,:]==0] = 0
    left = (((close(code,end_date).diff(7).abs()).iloc[-60:,:].rank(axis=0, pct=True)*(-1)).iloc[-20:,:] * sign.iloc[-20:,:]).iloc[-20:,:]
...
def alpha_181(code,benchmark='000300.XSHG',end_date=None):
    ...
    temp_2 = benchmark_index_close(code,benchmark,end_date)-pd.rolling_mean(benchmark_index_close(code,benchmark,end_date),20)
...
def alpha_184(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    part_1 = pd.rolling_corr((open(code,end_date)-close(code,end_date)).shift(),close(code,end_date),200).rank(pct=True)
...
def alpha_191(code,end_date=None):
    ...
    if isinstance(code, six.string_types):
        code = [code]
    alpha = pd.rolling_corr(pd.rolling_mean(volume(code,end_date),20),low(code,end_date),5)+(high(code,end_date)+low(code,end_date))/2-close(code,end_date)
    alpha = alpha.iloc[-1,:] 
    return alpha
